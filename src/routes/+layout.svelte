...
<div class="bg-blue-950 min-h-screen text-blue-100">
  <header class="bg-blue-900 flex justify-between p-8 items-center">
    ...
    <button on:click={toggleSettings} class="text-2xl bg-slate-800 p-2 hover:bg-slate-900 transition-colors">⚙️</button>
  </header>
  {#if showSettings}
    <div class="w-[70%] h-[70%] left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] absolute bg-slate-900 p-8">
      <div class="flex items-center justify-between">
        ...
        <button class="p-2 bg-slate-800 hover:bg-slate-700 transition-colors" on:click={toggleSettings}>❌</button>
      </div>
      {#if $signedIn}
